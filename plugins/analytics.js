...
import {extractDomain, getParamsFromUrl} from "~/helpers/UrlHelper"
import {getCookie, setCookie} from "~/helpers/CookieHelper"
import mixpanel from 'mixpanel-browser'
import {initTracking} from "~/services/TrackingEventService";
...
export default defineNuxtPlugin({
    ...
    hooks: {
        ...
        async 'app:mounted'() {
            ...
            setCookie('is_mobile', isMobileDevice())
            if (params?.['utm_source']?.length > 0) {
                setCookie('utm_source', params?.['utm_source'])
            }
            if (params?.['utm_medium']?.length > 0) {
                setCookie('utm_medium', params?.['utm_medium'])
            }
            if (params?.['utm_term']?.length > 0) {
                setCookie('utm_term', params?.['utm_term'])
            }
            if (params?.['utm_content']?.length > 0) {
                setCookie('utm_content', params?.['utm_content'])
            }
            if (params?.['utm_campaign']?.length > 0) {
                setCookie('utm_campaign', params?.['utm_campaign'])
            }
            if (params?.['pub']?.length > 0) {
                setCookie('pub', params?.['pub'])
            }

            const isInclude = (domain, lstTerm) => {
                if (domain?.length > 0 && lstTerm?.length > 0) {
                    for (let term of lstTerm) {
            ...
            getIndexedDB('first_visit').then((first_visit) => {
                if (first_visit === null) {
                    setIndexedDB('first_visit', new Date().toISOString())
                } else {
                    setCookie('first_visit', first_visit)
                }
            })

            getIndexedDB('__visitor').then((__visitor) => {
                if (__visitor !== null) {
                    setCookie('visitor_id', __visitor?.visitor_id)
                }
            })
