...
const myTickets = ref([]);
const listMyTicketsLoading = ref(true);

const fetchMyTickets = async () => {
  listMyTicketsLoading.value = true;
  try {
    const [tickets, total] = await getMyTickets(currentPage.value - 1, 10, filter.value, sorter.value);
    myTickets.value = tickets;
    totalRow.value = total;
  } catch (error) {
    console.error('Error fetching tickets:', error);
  } finally {
    listMyTicketsLoading.value = false;
  }
};
