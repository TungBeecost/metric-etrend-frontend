...
const fetchReportData = async () => {
  ...
  try {
    ...
    if (res.slug.startsWith('thuong-hieu')) {
      res.name = 'Thương hiệu ' + upperFirst(res.name);
    }
    ...
    <a-modal v-model:open="openModalWaiting" width="400px" destroy-on-close :footer="null" @ok="handleOk">
      <div style="display: flex; flex-direction: column; align-items: center; padding-top: 16px">
        ...
        <div class="payment_info">
          <p style="font-size: 16px; font-weight: 400; text-align: center">Đang kiểm tra trạng thái giao dịch. Vui lòng
            không đóng trang này.</p>
        </div>
      </div>
    </a-modal>
  </div>
