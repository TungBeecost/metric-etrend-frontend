...
export default function usePayment() {
    const createPaymentTransaction = async (paymentMethod: string, itemCode: string, redirectUrl: string, totalPrice: string, discountCode: string | null,  name: string | null, phone: string | null, emailAccount: string | null, company: string | null, tax_code: string | null, receive_email: string | null, address: string | null) => {
        try {
            return await createTransaction(paymentMethod, itemCode, redirectUrl, totalPrice, discountCode, name, phone, emailAccount, company, tax_code, receive_email, address);
        } catch (error) {
